...
public class SIDIDInfoActivity extends AppCompatActivity implements ItemSelectedInterface {
  ...
  public void applyChoice(Object object) {
    if (object instanceof CCPCountry) {
      ...
      mTvInputCountry.setTextColor(
          Color.parseColor(mSelectedCountryName.isEmpty() ? "#9AA6AC" : "#252C32"));
      ...
      mTvInputIdType.setTextColor(
          Color.parseColor(mSelectedIdCard.isEmpty() ? "#9AA6AC" : "#252C32"));
      ...
      findViewById(R.id.tvLblFirstName).setVisibility(showExtraFields ? View.VISIBLE : View.GONE);
      findViewById(R.id.edtFirstName).setVisibility(showExtraFields ? View.VISIBLE : View.GONE);
      findViewById(R.id.tvLblLastName).setVisibility(showExtraFields ? View.VISIBLE : View.GONE);
      findViewById(R.id.edtLastName).setVisibility(showExtraFields ? View.VISIBLE : View.GONE);
      findViewById(R.id.tvLblDoB).setVisibility(showExtraFields ? View.VISIBLE : View.GONE);
      findViewById(R.id.tvInputDoB).setVisibility(showExtraFields ? View.VISIBLE : View.GONE);
  ...
  public void go2Next(View view) {
    if (!isIdInfoValid()) {
      Toast.makeText(
              this, getString(R.string.sid_info_screen_incomplete_info_error), Toast.LENGTH_LONG)
          .show();
  ...
  private boolean isIdInfoValid() {
    if (mTvInputCountry
        .getText()
        .toString()
        .equalsIgnoreCase(getString(R.string.lbl_enroll_result_country))) return false;
    ...
    if (mEdtIdNbr
        .getText()
        .toString()
        .equalsIgnoreCase(getString(R.string.sid_info_screen_input_id_type_lbl))) return false;
    ...
    if (showExtraFields) {
      ...
      if (mTvInputDoB
          .getText()
          .toString()
          .equalsIgnoreCase(getString(R.string.sid_info_screen_input_dob_hint))) return false;
